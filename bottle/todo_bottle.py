...
def edit_item(no):
    ...
    if request.GET.save:
        ...
        return template("menu.tpl") + '<p>The item number %s was successfully updated</p>' % no
    else:
        conn = sqlite3.connect(DATABASE_FILE)
        c = conn.cursor()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DATABASE_FILE):
        conn = sqlite3.connect(DATABASE_FILE) # Warning: This file is created in the current directory
